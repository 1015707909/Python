...
def computeDistance(P1, P2, im, leftPoints, rightPoints):
    """
    根据输入的点得坐标，根据三角化法和视差法分别求解两点之间的实际距离,计算距离只与匹配点的坐标有关，与图像无关
    :param P1: 相机参数
    :param P2: 相机参数
    :im:左视图
    :param leftPoints: 两个点坐标，在左视图选取的点得坐标
    :param rightPoints: 与左视图选取的点在右视图中匹配到的点的坐标
    ...
    leftMax1 = get8Max(leftPoint1, r)
    # 左边点2的领域
    leftPoint2 = leftPoints[1]
    leftMax2 = get8Max(leftPoint2, r)

    # 右边点1的领域
    rightPoint1 = rightPoints[0]
    rightMax1 = get8Max(rightPoint1, r)
    # 右边点2的领域
    rightPoint2 = rightPoints[1]
    rightMax2 = get8Max(rightPoint2, r)
    ...
    for i in range(0, len(projLeftPoints)):
        ...
        # print "triangulatePoints4D:",triangulatePoints4D
        # print triangulatePoints4D
        # 当前点对的空间距离
        tempDis_t = np.linalg.norm(triangulatePoints4D[:, 0] - triangulatePoints4D[:, 1])
        # print "tempDis_t",tempDis_t
        # 把空间距离保存
